...
float4 BulkLightFragment(Varyings input) : SV_TARGET{
	float currentDepth = SAMPLE_DEPTH_TEXTURE_LOD(_CameraDepthTexture, sampler_point_clamp, input.screenUV, 0);
	float3 bulkLight = GetBulkLight(currentDepth, input.screenUV);
