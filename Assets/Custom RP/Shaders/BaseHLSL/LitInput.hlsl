...
UNITY_INSTANCING_BUFFER_START(UnityPerMaterial)
	...
	#ifdef _USE_BSDF         //使用BSDF的特殊情况
		UNITY_DEFINE_INSTANCED_PROP(float, _Wrap)
		UNITY_DEFINE_INSTANCED_PROP(float4, _ShiftColor)
		UNITY_DEFINE_INSTANCED_PROP(float, _FLTDistorion)
		UNITY_DEFINE_INSTANCED_PROP(float, _ILTPower)
		UNITY_DEFINE_INSTANCED_PROP(float, _FLTScale)
		UNITY_DEFINE_INSTANCED_PROP(float, _WidthControl)
		UNITY_DEFINE_INSTANCED_PROP(float4, _TransferColor)
	#endif
...
#ifdef _USE_BSDF         //使用BSDF的特殊情况
	float GetWrap(){
		return INPUT_PROP(_Wrap);
	}

	float3 GetShifColor(){
		return INPUT_PROP(_ShiftColor).rgb;
	}

	float GetDistorion(){
		return INPUT_PROP(_FLTDistorion);
	}

	float GetPower(){
		return INPUT_PROP(_ILTPower);
	}

	float GetScale(){
		return INPUT_PROP(_FLTScale);
	}

	float GetWidth(){
		return INPUT_PROP(_WidthControl);
	}

	float3 GetTransferColor(){
		return INPUT_PROP(_TransferColor).rgb;
	}
#endif
