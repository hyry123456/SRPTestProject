...
//不管是否支持阴影，直接进行阴影
Light GetDirectionalLightByPosition (int index, float3 worldPos, ShadowData shadowData) {
	Light light;
	light.color = _DirectionalLightColors[index].rgb;
	light.direction = _DirectionalLightDirectionsAndMasks[index].xyz;
	light.renderingLayerMask = asuint(_DirectionalLightDirectionsAndMasks[index].w);
	DirectionalShadowData dirShadowData =
		GetDirectionalShadowData(index, shadowData);
	light.attenuation =
		GetDirShadowByPosition(dirShadowData, shadowData, worldPos);
	return light;
}
...
Light GetOtherLightByPosition (int index, float3 worldPos, ShadowData shadowData) {
	...
	light.attenuation =
		GetOtherShadowByPosition(otherShadowData, shadowData, worldPos) *
		spotAttenuation * rangeAttenuation / distanceSqr;
	return light;
}
