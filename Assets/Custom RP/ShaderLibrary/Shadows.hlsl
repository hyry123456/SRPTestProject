...
ShadowMask GetDiffuseShadowMask(){
...
ShadowData GetShadowDataByPosition (float3 worldPos, float dither = 1) {
	ShadowData data;
	data.shadowMask.always = false;
	data.shadowMask.distance = false;
	data.shadowMask.shadows = 1.0;
...
float GetCascadedShadowByPosition (
	DirectionalShadowData directional, ShadowData global, float3 worldPos
) {
	float3 positionSTS = mul(
		_DirectionalShadowMatrices[directional.tileIndex],
		float4(worldPos, 1.0)
...
//不管是什么数据，只判断该点是否是阴影范围
float GetDirShadowByPosition(DirectionalShadowData directional, ShadowData global, float3 worldPos){
	float shadow;
	shadow = GetCascadedShadowByPosition(directional, global, worldPos);
	return shadow;
}


float GetOtherShadowByPosition (
	...
	float4 tileData = _OtherShadowTiles[tileIndex];
	float3 surfaceToLight = other.lightPositionWS - worldPos;
	float distanceToLightPlane = dot(surfaceToLight, lightPlane);
	float4 positionSTS = mul(
		_OtherShadowMatrices[tileIndex],
		float4(worldPos, 1.0)
