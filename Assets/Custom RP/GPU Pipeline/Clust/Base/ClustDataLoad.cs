    ...
    public class ClustDataLoad
    {
        ...
        public static List<BoundsData> LoadBoundsData(List<ClustData> clustDatas)
        {
            ...
            foreach (ClustData clustData in clustDatas)
            {
                ...
                boundsData.boundMin = clustData.boundMin;
        ...
        private static Vector3[] GetVector3(string strs)
        {
            ...
            Vector3[] result = new Vector3[positions.Count - 1];
        ...
        {
            List<string> positions = new List<string>(strs.Split('|'));
            Vector2[] result = new Vector2[positions.Count - 1];
            for (int i = 0; i < positions.Count - 1; i++)
            {
                string[] vs = positions[i].Split(',');
        ...
        {
            List<string> positions = new List<string>(strs.Split('|'));
            Vector4[] result = new Vector4[positions.Count - 1];
            for (int i = 0; i < positions.Count - 1; i++)
            {
                string[] vs = positions[i].Split(',');
        ...
        public static ClustData LoadOneClustData(string oneClustData)
        {
            string[] lines = GetEveryLine(oneClustData);
            ClustData clustData = new ClustData();
            clustData.Positions = GetVector3(lines[0]);
            Vector3[] vs = GetVector3(lines[1]);
            clustData.boundMin = vs[0];
            clustData.boundMax = vs[1];
            clustData.Normals = GetVector3(lines[2]);
            clustData.Tangents = GetVector4(lines[3]);
            clustData.UV0s = GetVector2(lines[4]);
            return clustData;
        }
    }
}