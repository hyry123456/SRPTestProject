...
float3 GetCubeBeginPos(float3 random, float3 cubeRange){
    float3 beginPos = -cubeRange / 2.0;
    float3 endPos = cubeRange / 2.0;
    return lerp(beginPos, endPos, random);
}
...
NoiseParticleData InitialParticle(NoiseParticleData i, Par_Initi_Data init) {
    switch (init.InitEnum.x){
        case 0:
            ...
            break;
        case 1:
            i.worldPos = GetSphereBeginPos(i.random.xy, init.sphereData.x, init.sphereData.y) + init.beginPos;
            break;
        case 2:
            i.worldPos = GetCubeBeginPos(i.random.xyz, init.cubeRange) + init.beginPos;
            break;
    }
    //改变取值位置，让速度与位置不要这么随机
...
NoiseParticleData OutParticle(NoiseParticleData i, Par_Initi_Data init){
    float time_01 = i.random.w / i.liveTime;
