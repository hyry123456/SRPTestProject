...
public class Lighting {
	...
	public void Setup (
		...
		SetupLights(useLightsPerObject, renderingLayerMask);	//注册灯光，传递灯光数据
		shadows.Render();										//渲染阴影
	...
	void SetupLights (bool useLightsPerObject, int renderingLayerMask) {
		...
		for (i = 0; i < visibleLights.Length; i++) {
			...
			if ((light.renderingLayerMask & renderingLayerMask) != 0) {	//判断该灯光是否要在本摄像机中支持
				switch (visibleLight.lightType) {			//判断灯光类型，进行对应的初始化
